...
def verdict_of(returncode: int, output: str) -> str:
    ...
    # THIS COPY IS STRICTER THAN THE SECRETS SKILL'S, and the difference is the
    # point. An ERROR is an exception that escaped the case: a SyntaxError or a
    # NameError the needle itself left in the engine scores exactly like that,
    # so an error says the needle broke something, not that the test noticed the
    # behaviour it names. Only a FAILURE is a verdict. The cases use
    # Guarded.refuses() instead of assertRaises for this reason: a wrong
    # exception class fails there instead of escaping.
    counts = re.search(r"^FAILED \((.*)\)$", output, re.M)
    failures = errors = 0
    if counts:
        for part in counts.group(1).split(","):
            ...
            if key == "failures":
                ...
                errors = int(value)
    if errors:
        return (f"the named test ERRORED ({errors} error(s)) instead of failing: an exception "
                f"escaped the case, which a needle that merely broke the engine also causes")
    if not failures:
        return "the run failed without a failing assertion, so nothing measured this mutation"
    return RED
...
def main() -> int:
    ...
    for mutation in MUTATIONS:
        root = Path(tempfile.mkdtemp(prefix="objstore-mutate-"))
        workspace = root / "object-store"
        try:
