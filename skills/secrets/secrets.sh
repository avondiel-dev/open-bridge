#!/bin/sh
# Entry point of the secrets skill.
#
...
#
# This engine imports nothing but the standard library, so the usual failure
# (an interpreter without PyYAML) cannot happen here. What can happen is the
...
refuse() {
    echo "secrets: no python3 here can run this skill." >&2
    echo >&2
