...
| `workflow/workloads/` | one declared run on one machine | core: template, schema, contract tests; user: entries | open-bridge (CORE)<br>your user branch | `placement.host` → infra/remotes/<br>`persona_ref` → identity/personas/ | /workload | when named |
